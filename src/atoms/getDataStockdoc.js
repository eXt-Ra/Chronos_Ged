...
export default function (numDoc) {
    return new Promise((resolve, reject) => {
        conn.pool.getConnection((err, connection) => {
            if (err) {
                ...
                //".$_POST['list'][$i]."
                connection.query(`select numdon,numimage,nopage,numenreg,taille,type,val4,val5 from STOCKDOC where val1 ='${numDoc}' and val1 is not null and taille>='232'`, function (err, lines) {
                    if (err) {
                        throw err;
