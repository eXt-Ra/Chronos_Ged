...
export default function traitFileRetour(position, remettant) {
    ...
    return new Promise((resolve, reject) => {
        ...
        fileTypeCheck(position.documents[0].filePath).then(type => {
            ...
            const promiseQ = [];
            mkdirp(`${position.documents[0].currentFileLocation}/retour`, () => {
                files.forEach(file => {
                    switch (conf.nomenclature) {
                        case "numeroEquinoxe_date":
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `${position.numEquinoxe}_${moment().format()}${file.substr(file.length - 4)}`)), () => {
                        ...
                        case "numeroEquinoxe_date_page":
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `${position.numEquinoxe}_${moment().format()}_${file.charAt(0)}${file.substr(file.length - 4)}`)), () => {
                        ...
                        case "PA-numeroEquinoxe_page" :
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `PA-${position.numEquinoxe}_${file.charAt(0)}${file.substr(file.length - 4)}`)), () => {
                        ...
                        case "numeroEquinoxe-page" :
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `${position.numEquinoxe}-${file.charAt(0)}${file.substr(file.length - 4)}`)), () => {
                        ...
                        case "RE-refTMS_page" :
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                getRefTMS(position).then(refTMS => {
                                    fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `RE-${refTMS}_${file.charAt(0)}${file.substr(file.length - 4)}`)), () => {
                        ...
                        case "refTMS-page" :
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                getRefTMS(position).then(refTMS => {
                                    fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `${refTMS}-${file.charAt(0)}${file.substr(file.length - 4)}`)), () => {
                        ...
                        case "numeroEquinoxe-refDitri_page" :
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `${position.numEquinoxe}-${position.codeEdi}_${file.charAt(0)}${file.substr(file.length - 4)}`)), () => {
                        ...
                        case "numeroEquinoxe-refDitri-refTMS_page" :
                            promiseQ.push(new Promise((resolve2, reject2) => {
                                getRefTMS(position).then(refTMS => {
                                    fs.rename(path.join(position.documents[0].currentFileLocation, file), path.join(position.documents[0].currentFileLocation, path.join("retour", `${position.numEquinoxe}-${position.codeEdi}-${refTMS}_${file.charAt(0)}${file.substr(file.length - 4)}`)), () => {
