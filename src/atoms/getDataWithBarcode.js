  ...
  return new Promise((resolve, reject) => {
	conn.pool.getConnection((err, connection) => {
		  if (err) {
			...
			connection.query(`SELECT s2.PROPRIETE ,s2.NUM_DOC FROM search_doc s1 INNER JOIN search_doc s2 ON s1.NUM_DOC = s2.NUM_DOC INNER JOIN DOCUMENT ON DOCUMENT.NUM_DOC=S1.NUM_DOC WHERE s1.PROPRIETE = '${numEquinoxe}' AND s2.PROPRIETE != 'GENTFRO' AND s2.NUM_CHAMPS=12 order by document.dATECREATION desc`, function (err, lines) {
			  if (err) {
